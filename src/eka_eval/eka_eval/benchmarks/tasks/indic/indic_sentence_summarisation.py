# eka_eval/benchmarks/tasks/indic/indic_sentence_summarization.py
import torch
import os
...
DEFAULT_DATASET_NAME = "ai4bharat/IndicSentenceSummarization"
...
DEFAULT_PROMPT_TEMPLATE_KEY_ZERO_SHOT = "indic_sentence_summarization_0shot"
DEFAULT_PROMPT_TEMPLATE_KEY_FEW_SHOT = "indic_sentence_summarization_5shot"
...
def save_detailed_results(
    ...
    filename = f"indic_sent_summ_{model_clean}_{dataset_clean}_{language}_p{process_id}_{timestamp}.json"
...
def _create_prompt(text: str, language: str, prompt_template_dict: Dict, few_shot_examples: List[Dict] = None) -> str:
...
def evaluate_indic_sentence_summarization(
    pipe: Any, 
    tokenizer: Any, 
    ...
    **kwargs
    ...
    logger.info(f"Starting IndicSentenceSummarization ({num_few_shot}-shot): {model_name_for_logging}")
    ...
    if not prompt_template_dict:
        ...
        return {"IndicSentenceSummarization": 0.0, "error_message": f"PromptTemplate '{current_prompt_template_name}' NotFound"}
    ...
    for lang_code in target_languages:
        ...
        try:
            ...
            for example_idx, example in enumerate(tqdm(dataset, desc=f"Eval {lang_code}")):
                ...
                prompt = _create_prompt(text, lang_code, prompt_template_dict, few_shot_examples_to_use)
            ...
            if rouge1_scores:
                lang_rouge1 = np.mean(rouge1_scores)
                lang_rouge2 = np.mean(rouge2_scores)
                lang_rougeL = np.mean(rougeL_scores)
                lang_bleu = np.mean(bleu_scores)
    ...
    final_scores = {
        "IndicSentenceSummarization_ROUGE1": overall_rouge1,
        "IndicSentenceSummarization_ROUGE2": overall_rouge2,
        "IndicSentenceSummarization_ROUGEL": overall_rougeL,
        "IndicSentenceSummarization_BLEU": overall_bleu
    }
    
    for lang, scores in language_scores.items():
        if scores:
            final_scores[f"IndicSentenceSummarization_{lang}_ROUGE1"] = scores["rouge1"]
            final_scores[f"IndicSentenceSummarization_{lang}_ROUGE2"] = scores["rouge2"]
            final_scores[f"IndicSentenceSummarization_{lang}_ROUGEL"] = scores["rougeL"]
            final_scores[f"IndicSentenceSummarization_{lang}_BLEU"] = scores["bleu"]
